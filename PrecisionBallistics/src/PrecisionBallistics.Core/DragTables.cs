...
/// <summary>Standard projectile drag model.</summary>
public enum DragModel
{
    /// <summary>G1 (flat-base, blunt) standard reference projectile.</summary>
    G1,
    /// <summary>G7 (long boat-tail) standard reference projectile — preferred for modern VLD/match bullets.</summary>
    G7,
    /// <summary>Custom Doppler-derived drag curve (per-projectile Mach→Cd table). Falls back to G7 if no table is set.</summary>
    Custom
}

/// <summary>
/// Standard drag-coefficient tables for the G1 and G7 reference projectiles.
/// Values are the published BRL standard drag functions (Mach number -> Cd),
/// which are public-domain reference data. The actual projectile's drag is
/// scaled from these by its ballistic coefficient inside the solver.
/// </summary>
public static class DragTables
{
    ...
    /// <summary>Linearly interpolated standard drag coefficient for the given model at the given Mach number.</summary>
    public static double Cd(DragModel model, double mach)
    {
        var t = model == DragModel.G7 ? G7Table : G1Table;
        if (mach <= t[0].Mach) return t[0].Cd;
        if (mach >= t[^1].Mach) return t[^1].Cd;
